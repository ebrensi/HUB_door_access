...
def parse_text_report(text_file):
    ...
    p_user = re.compile(
        '^\s*(\d+:\d+ [AP]M) .* (ENTRY|EXIT) READER .* User: (\d+) (.*) ')
    ...
    df = (pd
          .DataFrame(zip(year, date, time, tstamp, uid, name, loc),
                     columns=['Year',
                              'Date',
                              'Time',
                              'tstamp',
...
def make_durations(df):
    ...
    # check == 1 where Loc is OUTSIDE followed by INSIDE
    check = loc_ind.diff()

    # We must disregard mistaken positives coming from
    #   OUTSIDE from one person followed by INSIDE from another person
    # First row of every UID group (Name)
    firsts = df.groupby('UID').head(1).index
    check[firsts] = 0
...
def pivot_by_user(df):
    ...
    #  we use 'last' as aggfunc.  This is hardly necessary but
    #  occasionally two events register
...
def group_by_user(df1, fname=False):
    ...
    df_user = (df2
               ...
               .set_index(['UID',
                           ...
                           'Date',
                           'Day',
                           'Time'])[['tstamp', 'Loc', 'Period']])
    df_user['tstamp'] = (df_user
                         .tstamp
                         .map(lambda x: x.strftime('%Y/%m/%d %H:%M')))
    if fname:
...
#"""
df = pd.read_csv('HUB_Door_all.csv',
                 names=['tstamp', 'Loc', 'UID', 'Name'],
                 skiprows=[0])
df.tstamp = pd.to_datetime(df.tstamp)
df.Loc = df.Loc.astype('category')
...
print('{} unique user IDs'.format(df['UID'].nunique()))
#"""

#"""
# Next we resample time series to one day intervals and indicate
#  whether a user swiped a card that day
